...
// -----------------------------------------------------------------------------
// Responsive scaling for the T-Watch Ultra
// The original layout was tuned for the 240x240 S3 panel. The Ultra AMOLED is
// 410x502, so we scale fixed pixel dimensions from that 240px design baseline
// using the real display resolution reported by LVGL. Percentage-based widths
// already adapt on their own; SC() rescales the fixed heights/offsets.
// -----------------------------------------------------------------------------
static const int32_t UI_DESIGN_WIDTH = 240;

static float ui_scale_factor() {
    static float cached = 0.0f;
    if (cached <= 0.0f) {
        int32_t hor = lv_disp_get_hor_res(NULL);
        if (hor <= 0) hor = UI_DESIGN_WIDTH;
        cached = static_cast<float>(hor) / static_cast<float>(UI_DESIGN_WIDTH);
        if (cached < 1.0f) cached = 1.0f; // never shrink below the S3 baseline
    }
    return cached;
}

// Scale a design pixel value to the current panel.
static lv_coord_t SC(int32_t design_px) {
    return static_cast<lv_coord_t>(design_px * ui_scale_factor() + 0.5f);
}

// Pick a larger Montserrat step on big panels. Only fonts already used by the
// project (14/16/18/20/24/36) are referenced so no extra fonts must be enabled
// in lv_conf.h.
static const lv_font_t *ui_font(const lv_font_t *base) {
    if (ui_scale_factor() < 1.3f) return base;
    if (base == &lv_font_montserrat_14) return &lv_font_montserrat_20;
    if (base == &lv_font_montserrat_16) return &lv_font_montserrat_24;
    if (base == &lv_font_montserrat_18) return &lv_font_montserrat_24;
    if (base == &lv_font_montserrat_20) return &lv_font_montserrat_24;
    if (base == &lv_font_montserrat_24) return &lv_font_montserrat_36;
    if (base == &lv_font_montserrat_36) return &lv_font_montserrat_36;
    return base;
}
...
static lv_obj_t *create_tasks_screen() {
    ...
    lv_obj_set_size(tasks_list, lv_pct(92), SC(130));
    ...
    lv_obj_set_size(back_btn, SC(112), SC(48));
...
static lv_obj_t *create_calendar_screen() {
    ...
    lv_obj_set_size(events_list, lv_pct(92), SC(108));
...
static lv_obj_t *create_gym_screen() {
    ...
    if (cached_gym_session.id > 0) {
        ...
        lv_obj_set_size(list, lv_pct(100), SC(130));
    ...
    // Back button
    lv_obj_t *back_btn = lv_btn_create(screen);
    lv_obj_set_size(back_btn, SC(112), SC(46));
...
static lv_obj_t *create_gym_set_screen(int exercise_log_id, const char *exercise_name) {
    ...
    // Back button
    lv_obj_t *back_btn = lv_btn_create(screen);
    lv_obj_set_size(back_btn, SC(100), SC(42));
    lv_obj_align(back_btn, LV_ALIGN_BOTTOM_MID, 0, -4);
...
static lv_obj_t *create_shopping_screen() {
    ...
    if (shopping_item_count > 0) {
        ...
        lv_obj_t *list = lv_obj_create(screen);
        lv_obj_set_size(list, lv_pct(100), SC(140));
...
static lv_obj_t *create_lock_screen() {
    ...
    lv_obj_set_width(lock_current_activity_label, SC(220));
    lv_obj_align(lock_current_activity_label, LV_ALIGN_BOTTOM_MID, 0, SC(-40));

    lock_next_activity_label = lv_label_create(screen);
    lv_label_set_text(lock_next_activity_label, "Next: --");
    lv_obj_set_style_text_font(lock_next_activity_label, ui_font(&lv_font_montserrat_14), 0);
    ...
    lv_obj_set_width(lock_next_activity_label, SC(220));
    lv_obj_align(lock_next_activity_label, LV_ALIGN_BOTTOM_MID, 0, SC(-22));

    lv_obj_t *hint = lv_label_create(screen);
    lv_label_set_text(hint, "Tap to unlock");
    lv_obj_set_style_text_font(hint, ui_font(&lv_font_montserrat_14), 0);
    lv_obj_set_style_text_color(hint, lv_palette_lighten(LV_PALETTE_GREY, 1), 0);
    lv_obj_align(hint, LV_ALIGN_BOTTOM_MID, 0, SC(-4));
...
void init_ui() {
    ...
    lv_obj_set_style_text_font(title, ui_font(&lv_font_montserrat_18), 0);
    lv_obj_set_style_text_color(title, lv_palette_lighten(LV_PALETTE_GREY, 2), 0);
    lv_obj_align(title, LV_ALIGN_TOP_MID, 0, 0);
    ...
    lv_obj_set_size(grid, lv_pct(100), SC(196));
    lv_obj_align(grid, LV_ALIGN_TOP_MID, 0, SC(28));
    lv_obj_set_layout(grid, LV_LAYOUT_GRID);
    lv_obj_set_grid_dsc_array(grid, columns, rows);
    lv_obj_set_style_pad_all(grid, SC(4), 0);
    lv_obj_set_style_pad_row(grid, SC(6), 0);
    lv_obj_set_style_pad_column(grid, SC(6), 0);
    ...
    for (size_t index = 0; index < (sizeof(MENU_ITEMS) / sizeof(MENU_ITEMS[0])); ++index) {
        ...
        lv_obj_set_style_text_font(icon_label, ui_font(&lv_font_montserrat_24), 0);
